...
class Repository:
    ...
    def __init__(
        self, model: Any,
        connection: AsyncIOMotorClient  # type: ignore
    ):
        """Initialize the repository.

        Args:
            model (Any): The Pydantic model representing
        ...
        """
        self.model = model
        self.database = connection['chickie']  # type: ignore
        self.collection = self.database[model.__tablename__]

    async def find_one(self, **kwargs) -> Optional[Any]:
        """Find a single document in the MongoDB collection based
        on given criteria.
        ...
        Returns:
            Optional[Any]: The Pydantic model instance representing
    ...
    async def find_all_containing(self, **kwargs) -> List[Any]:
        """Find all documents in the MongoDB collection containing
        the given values.
        ...
        cursor: AgnosticCursor = await self.collection.find(
            query  # type: ignore
        )
        results = cursor.to_list(None)  # type: ignore
    ...
    async def save(self, model: Any) -> str:
        ...
        Args:
            model (Any): The Pydantic model instance representing the
    ...
    async def update(self, item: Any, data: dict = {}) -> int:
        ...
        Args:
            item (Any): The Pydantic model instance representing the document
            to be updated.
    ...
    async def delete(self, item: Any) -> int:
        ...
        Args:
            item (Any): The Pydantic model instance representing the
