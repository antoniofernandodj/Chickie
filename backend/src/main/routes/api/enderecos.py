...
async def requisitar_endereco(
    uuid: Annotated[
        str, Path(title="O uuid do endereço a fazer get")
    ]
):
    async with DatabaseConnectionManager() as connection:
        repository = Repository(connection=connection)
...
async def atualizar_endereco_patch(
    uuid: Annotated[
        str, Path(title="O uuid do endereço a fazer patch")
    ]
):
...
async def atualizar_endereco_put(
    itemData: Endereco,
    uuid: Annotated[
        str, Path(title="O uuid do endereco a fazer put")
    ]
):
    async with DatabaseConnectionManager() as connection:
        repository = Repository(connection=connection)
...
async def remover_endereco(
    uuid: Annotated[
        str, Path(title="O uuid do endereço a fazer delete")
    ]
):
    async with DatabaseConnectionManager() as connection:
        repository = Repository(connection=connection)
